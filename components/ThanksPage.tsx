...
const ThanksPage: React.FC<ThanksPageProps> = ({ navigateTo, t, registerAgainTarget }) => {
    return (
        <div className="text-center py-16">
            ...
            <h2 className="mt-6 text-3xl font-bold text-stone-800 dark:text-stone-100">{t('thanksMessage')}</h2>
            <p className="mt-2 text-stone-600 dark:text-stone-400">{t('thanksText')}</p>
            <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
                <button onClick={() => navigateTo(registerAgainTarget)} className="bg-stone-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-stone-700 transition-all">{t('registerAnotherBtn')}</button>

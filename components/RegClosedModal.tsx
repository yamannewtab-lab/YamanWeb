...
const RegClosedModal: React.FC<RegClosedModalProps> = ({ t, onClose }) => {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-8 max-w-sm w-full text-center">
                <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">{t('regClosedTitle')}</h3>
                <p className="mt-4 text-slate-600 dark:text-slate-300">{t('regClosedText')}</p>

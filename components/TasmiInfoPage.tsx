...
const TasmiInfoPage: React.FC<TasmiInfoPageProps> = ({ navigateTo, t }) => {
    return (
        <div className="text-center py-16">
            <div className="inline-block bg-sky-100 dark:bg-sky-900/50 p-4 rounded-full">
            ...
            <p className="mt-4 text-slate-600 dark:text-slate-300">{t('tasmiInfoText1')}</p>
            <p className="mt-2 text-slate-600 dark:text-slate-300">{t('tasmiInfoText2')}</p>
            <div className="mt-8 bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg max-w-md mx-auto">
                <p className="font-bold text-slate-800 dark:text-slate-200">{t('noteTitle')}</p>
                <p className="text-slate-600 dark:text-slate-300">{t('noteText')}</p>
            </div>
            <div className="mt-12">
